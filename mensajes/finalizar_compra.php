<?php 
	...
	if (isset($_GET['aprobado']) && $_GET['aprobado'] == 'true') {
		...
		$sel = $con->prepare("SELECT clave_producto, cantidad FROM compras WHERE correo_usuario = ? AND status_compra = 'AGREGADO' ");
		...
		while ($f = $sel->fetch()) { 
	  	...
	  	$fecha = date('Y-m-d');
	  	$up = $con->prepare("UPDATE compras SET status_envio = 'POR ENVIAR', status_compra = 'COMPRADO', fecha = :fecha WHERE correo_usuario = :correo AND status_compra = 'AGREGADO' ");
